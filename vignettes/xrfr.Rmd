...
This package was created for use at the University of Bergen to convert data from an X-ray fluorescence machine from kilo-counts per second to micromolar. It requires that your files are constructed in a very specific way, as explained in this vignette, so is meant to be used with instructions. In case you wish to use the package but do not have access to the instructions, the instructions document is available [here](https://github.com/agryt/xrfr). 
...
* `widen_above()`: This function both widens your data like `widen()` does, and excludes any concentrations that are below the detection limits. 
* `widen_means()`: This function both widens your data like `widen()` does, and calculates mean concentration based on one or two factors. 
