...
* `show_blanks()`: This function creates a dataframe showing only the mean blanks in your dataset. 
* `transformssd()`: This function transforms hexadecimal values (obtained from a .ssd file created by the XRF computer) to a dataframe showing results from one sample. The dataframe will be in the same format as the .txt file created by the XRF computer, so it is possible to then use the `readxrf()` and `convertxrf()` functions on this dataframe. 
...
This function calculates the mean values for the blank values based on filter type, filter size, and filter box number, and presents these in a dataframe. There are 3 arguments that must be defined:

* **imported_data**: The name of the dataframe created with `readxrf()`. 
* **first_element**: The name of the first column that is an element in the project dataframe.
* **last_element**: The name of the last column that is an element in the project dataframe.
...
The created dataframe will contain the mean blank values and look something like [this](LINK):

|||

### transformssd()

This function will convert the hexadecimal values from a .txt file (created from a .ssd file from the XRF computer) to a dataframe with results from one sample. This dataframe can then be used in the `readxrf()` function. There is one argument that must be defined for this function:

* **hex_data**: The name of the .txt file containing the hexadecimal values. 

A possible input:

```{r transformssd, eval = FALSE}

df <- transformssd(hex_data = "sample_hex.txt")

```

The created dataframe will look like [this](https://github.com/agryt/xrfr/blob/master/inst/extdata/sample_transformed.csv):

|Sample|Date|As (Int)|C (Int)|...|Zn (Int)|
|:-----|:---|:-------|:------|:--|:-------|
|sr-cal120|27.11.2014 14:02:43|1.632752776145935|54.286903381347656|...|0.7868552803993225|

There will always be only be results from one sample in this dataframe, as each sample is saved as its own .ssd file. The different dataframes from samples in the same dataset can be combined into one dataframe using `rbind()`. 
