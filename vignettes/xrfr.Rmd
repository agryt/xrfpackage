...
All these files must be imported to RStudio as dataframes before you can use the xrfr package. Your raw data file will be a .txt file, which can be imported using the `read_delim()` function. This code should work: `read_delim("name_of_your_file.txt", delim = "\t", locale = readr::locale(decimal_mark = ","))`. How you import your other files depends on which format they are saved as. For Excel files, use the readxl package and the function `read_excel()`. For CSV files use either `read_csv()` (comma as separator and . as decimal point) or `read_csv2()` (tab as separator and , as decimal point) from the readr package. 
...
* **raw.data**: This is the name of your dataframe with raw data from the XRF machine. 
* **project.info**: This is the name of your dataframe with the project information. 
...
imported.df <- importxrf(raw.data = rawdata.df, project.info = projectinfo.df)
...
* **imported.data**: The name of the dataframe created after using `importxrf()`. 
* **setup**: This is the name of your dataframe with the "setup" information. 
...
data.df <- convertxrf(project.data = imported.df, setup = setup.df, year = "2019", first_element = "C", last_element = "As")

```

This code will give a long dataframe - you can see the first 96 rows [here](https://github.com/agryt/xrfr/blob/master/inst/extdata/project_calculated_sliced.csv). The table below shows the first few rows of this file:
...
The dataframe created here is a good idea to save, as this is the file you will likely use to perform statistical analyses, make plots, etc. You can save it as a CSV file by using the `write_csv()` function from the readr package (part of tidyverse). 

### widen()

The file created using `convertxrf()` is good for further work in R/Rstudio, but if you want to look at your data it is not ideal. The `widen()` function will show your data as is, only excluding the blanks and detection limits, and turning the dataframe from a long to a wide format. For this function to work you will need to define this argument:

* **project.data**: This is the name of the dataframe created using `convertxrf()`. 
...
Running this code will give you a dataframe that looks something like [this](https://github.com/agryt/xrfr/blob/master/inst/extdata/project_wide.csv):
...
This dataframe is a lot easier to read with human eyes than the long format. 

### widen_above()

In addition to widening your data, you may wish to exclude the values that are below the detection limits. This can be done with the `widen_above()` function. It will give you the same dataframe as the `widen()` function, only with "NA" or "NULL" written in the cells where the concentration was lower than the detection limit. You will need to define this argument:
...
The output of this code will be a dataframe that looks like [this](https://github.com/agryt/xrfr/blob/master/inst/extdata/project_wide_abovedetlim.csv):
...
* **project.data**: This is the name of the dataframe created using `convertxrf()`.
* **first_factor**: This is the name of the column that shows the first or only factor you want to calculate means based on, for example depth. 
* **second_factor**: *Defining this argument is optional.* This is the name of the column that shows the second factor you want to calculate means based on, for example location.
* **first_element**: This is the name of the first column containing concentration values in the project dataframe created using `convertxrf()`.
* **last_element**: This is the name of the last column containing concentration values in the project dataframe created using `convertxrf()`.

A possible input:

```{r widen_means, eval = FALSE}

df <- widen_means(project.data = data.df, first_factor = "Treatment", second_factor = "Day", first_element = "C", last_element = "As")

```

The output of this code will be a dataframe showing the factor(s) chosen and the mean concentration for each element, like [this](https://github.com/agryt/xrfr/blob/master/inst/extdata/project_wide_means.csv): 
