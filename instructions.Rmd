...
3. Import your data files to R using the appropriate functions (described under "Preparing your data" if you are unsure how to do this). 
...
```{r intro 2}
DATA.df <- convertxrf(imported_data = DATA0.df, base_info = YOURBASEINFO.df, year = "2019", first_element = "C", last_element = "As")
```
...
```{r intro 4}
means.project.df <- widen_means(project_data = data.df, first_factor = "Location", second_factor = "Depth")
```
...
```{r intro 6}
means.above.project.df <- widen_means_above(project_data = data.df, first_factor = "Location", second_factor = "Depth")
```
Note that "first_factor" must be defined while "second_factor" is optional. 
...
This will make three new data frames appear in your environment. At this point it is a good idea to open them (by clicking on them in the environment or running the code `view(NAMEOFDATAFRAME)`) and check that they were imported correctly. 
